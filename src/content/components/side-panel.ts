...
export class SidePanel extends LitElement {
  ...
  @state() _originalWord = '';
  @state() _isFavorited = false;
  @state() _isClosing = false;
  ...
  static styles = [
    ...
    css`
      ...
      :host([visible]) .panel { animation: slideIn 0.25s ease; }
  ...
  render() {
    if (!this.visible || !this.translation) return html``;
    ...
    return html`<div class="panel">
      ...
      ${t.phonetic ? html`<div class="phonetic">
        ...
        <button class="btn" @click="${this._speak}" style="padding:2px 6px;font-size:12px">🎵 朗读</button>
      </div>` : ''}
      ...
      <div class="actions">
        ...
        <button class="btn" @click="${this._switchSource}">🔄 换源</button>
  ...
  setFavorited(val: boolean) { this._isFavorited = val; }
  ...
  private _switchSource() {
    this.dispatchEvent(new CustomEvent('switch-source', {
      bubbles: true, composed: true,
    }));
  }
