...
- **LLM 知识增强释义**：DeepSeek 自动识别 单词 / 短语 / 专业术语 / 句子,只给出合适的字段——词形变化、同反义词、常用搭配、词根词缀、易混辨析、记忆技巧、语域;专业术语额外附**专业定义式百科解释**(如 "static checking 指在不运行程序的情况下由编译器检查代码")。弹泡显示内容可自定义。
...
- **长文本分块翻译**：最多 2 万字符，按句子边界自动分块（各源限额自适应），部分失败返回已有结果并标注
- 支持 中/英/日/韩/法/德/西 互译
- 多翻译源：DeepSeek、Google Translate、腾讯云 TMT、百度翻译、DeepL，浮层内一键换源
- **知识增强释义**：音标、词性、词形变化、同反义词、常用搭配、词根词缀、易混辨析、记忆技巧、语域；专业术语附**百科解释**。AI 按输入类型自动取舍字段——单词给全量知识，短语/术语不给词根词缀，句子只给译文。弹泡展示内容可在设置浮窗中自定义
- 快捷键 `Alt+T` 翻译、`Alt+R` 朗读、`Esc` 关闭

### AI 助手（就这一页提问）

- 弹泡图标 / 侧栏页签一键在「翻译 / 助手」之间切换，助手会话在弹泡与侧栏之间共享
- 自动注入当前页面上下文（围绕选中内容截取）与选中范围，可提问、追问、整页速览
- 流式输出 + 思考过程可折叠；脚注显示输入/输出 token、上下文缓存命中率与耗时
- 快捷提问：解释选中 / 说人话 / 给例子 / 考考我 / 这页讲了什么；**这些预设可自定义**——增删改、调顺序、改「是否需要选中」与「取上下文方式」，工具栏的整页类入口也从预设派生；答案可朗读、可复制
- `Alt+Q` 直接对选中内容提问
- 助手行为规则可改写（选项页），另有一条防提示词注入的安全兜底规则固定生效、不可修改
- 设置分两处：**选项页能调全部项**——上下文长度（0–32000 字，0 = 只带选中范围）、思考深度（关闭/低/高/最大）、助手行为规则、预设提问、回答长度上限；弹泡里的设置浮窗是其中「上下文长度 + 思考深度」的快捷入口（其余请到选项页调）。弹泡里改完即时生效，选项页下次打开时读到的是已存的新值
- 译文卡片记住最近 10 条查过的词，meta 行 `‹ ›` 可前后回退（直接回显缓存，不重新请求翻译源）；只在内存，刷新页面清空
